...
router.post('/definitions', (req, res) => {
  const { field_name, field_type, dropdown_options, sort_order, scope } = req.body;
  const r = db.prepare(`
...
router.put('/definitions/:id', (req, res) => {
  const { field_name, field_type, dropdown_options, sort_order, scope } = req.body;
  const prev = db.prepare('SELECT * FROM custom_field_definitions WHERE id = ?').get(req.params.id);
  if (!prev) return res.status(404).json({ error: 'Not found' });
  const r = db.prepare(`
    UPDATE custom_field_definitions SET field_name=?, field_type=?, dropdown_options=?, sort_order=?, scope=? WHERE id=?
  `).run(field_name, field_type || 'Text',
         dropdown_options ? JSON.stringify(dropdown_options) : null, sort_order ?? 0,
         scope || prev.scope || 'engagement', req.params.id);
...
// ── Contact-scoped custom field endpoints ─────────────────────────────────────
router.get('/contact-fields', (req, res) => {
  res.json(db.prepare(
    "SELECT * FROM custom_field_definitions WHERE scope='contact' ORDER BY sort_order ASC"
  ).all());
});

router.get('/contact-values/:contactId', (req, res) => {
  res.json(db.prepare(`
    SELECT ccfv.*, cfd.field_name, cfd.field_type, cfd.dropdown_options
    FROM contact_custom_field_values ccfv
    JOIN custom_field_definitions cfd ON cfd.id = ccfv.field_definition_id
    WHERE ccfv.contact_id = ?
  `).all(req.params.contactId));
});

router.post('/contact-values/:contactId', (req, res) => {
  const { field_definition_id, value } = req.body;
  if (!field_definition_id) return res.status(400).json({ error: 'field_definition_id required' });
  db.prepare(`
    INSERT INTO contact_custom_field_values (contact_id, field_definition_id, value)
    VALUES (?, ?, ?)
    ON CONFLICT(contact_id, field_definition_id) DO UPDATE SET value = excluded.value
  `).run(req.params.contactId, field_definition_id, value ?? null);
  res.json({ ok: true });
});
